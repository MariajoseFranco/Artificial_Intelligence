...
def autoencoder(features):
    ...
    encoded = Dense(6, activation='sigmoid')(input_layer)
...
if __name__ == '__main__':
    ...
    # High dimensions
    X_hd = autoencoder(xi)
    df_X_hd = pd.DataFrame(X_hd, columns = ['x1','x2','x3','x4','x5','x6','x7','x8','x9','x10','x11','x12'])
    df_y_hd = pd.DataFrame(yi, columns=['y1', 'y2'])
    df_X_hd = pd.concat([df_X_hd, df_y_hd], axis = 1)
    df_X_hd.to_excel(r'.\hd.xlsx', sheet_name='High dimension')

    # Low dimensions
    X_ld = umap_algorithm(xi)
    df_X_ld = pd.DataFrame(X_ld, columns = ['x1','x2','x3'])
    df_y_ld = pd.DataFrame(yi, columns=['y1', 'y2'])
    df_X_ld = pd.concat([df_X_ld, df_y_ld], axis = 1)
    df_X_ld.to_excel(r'.\ld.xlsx', sheet_name='Low dimension')
...
if __name__ == '__main__':
    ...
    x3 = df['MaxHR']
    y = df['HeartDisease']
    df_junto = np.transpose(np.array([x1,x2,x3,y]))
    df_new = pd.DataFrame(df_junto, columns = ['x1','x2','x3','y'])
    df_normalized=(df_new-df_new.min())/(df_new.max()-df_new.min())
    df_normalized = df_normalized.dropna()
    df_normalized = df_normalized.sample(frac=1)
    x1 = df_normalized["x1"]
    x2 = df_normalized["x2"]
    x3 = df_normalized["x3"]
    xi = np.transpose(np.array([x1,x2,x3]))
    y1 = df_normalized["y"]
    yi = np.transpose(np.array([y1]))
    # High dimensions
    X_hd = autoencoder(xi)
    df_X_hd = pd.DataFrame(X_hd, columns = ['x1','x2','x3','x4','x5','x6'])
    df_y_hd = pd.DataFrame(yi, columns=['y'])
    df_X_hd = pd.concat([df_X_hd, df_y_hd], axis = 1)
    df_X_hd.to_excel(r'.\hd_camilo.xlsx', sheet_name='High dimension')

    # Low dimensions
    X_ld = umap_algorithm(xi)
    df_X_ld = pd.DataFrame(X_ld, columns = ['x1','x2'])
    df_y_ld = pd.DataFrame(yi, columns=['y'])
    df_X_ld = pd.concat([df_X_ld, df_y_ld], axis = 1)
    df_X_ld.to_excel(r'.\ld_camilo.xlsx', sheet_name='Low dimension')
# %%
