...
if __name__ == '__main__':
    df = pd.read_excel(r'C:\Users\Asus\Documents\MAJO\Universidad\SEMESTRE 10\INTELIGENCIA ARTIFICIAL\Rice_Dataset_Commeo_and_Osmancik\Normalized_data.xlsx', header=0,  names=["x1", "x2", "x3", "x4", "x5", "x6"])
    x1 = df["x1"]
    x2 = df["x2"]
    x3 = df["x3"]
    x4 = df["x4"]
    x5 = df["x5"]
    x6 = df["x6"]
    xi = np.transpose(np.array([x1,x2,x3,x4,x5,x6]))
    ...
    # High dimensions
    X_hd = autoencoder(xi)
    df_X_hd = pd.DataFrame(X_hd, columns = ['x1','x2','x3','x4','x5','x6','x7','x8','x9','x10'])
    #df_y_hd = pd.DataFrame(yi, columns=['y1', 'y2'])
    #df_X_hd = pd.concat([df_X_hd, df_y_hd], axis = 1)
    df_X_hd.to_excel(r'C:\Users\Asus\Documents\MAJO\Universidad\SEMESTRE 10\INTELIGENCIA ARTIFICIAL\Rice_Dataset_Commeo_and_Osmancik\hd_camilo.xlsx', sheet_name='High dimension')
